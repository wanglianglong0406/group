...
public class DateUtil {
    ...
    //获得多少分钟后的时间  10 minut = 600000
    public static Date addDateMinut(Date now, Long time) {
    ...
    public static String createActionNo(Date now) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHH");
        String actionNo = sdf.format(now);
        if(actionNo.length()<11){
            for (int i = 0; i < 3; i++) {
                actionNo= actionNo + "0";
            }
        }
        Long a= Long.valueOf(actionNo);
        String b=String.valueOf(a);
        return b;

    }

    public static String dateToStringyyyyMMdd(Date now) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
        return sdf.format(now);

    }

    public static String dateToStringyyyyMMddhhmm(Date now) {
